  ...
  return (
    <div className="printReceipt rounded-lg border border-dashed border-slate-300 bg-white p-5 text-sm">
      ...
      <div className="mb-4 space-y-1 border-b border-dashed border-slate-300 pb-4">
        ...
        {(movement?.customer_name || movement?.customer_id) && (
          <div className="mt-2 rounded bg-slate-50 p-2 text-xs">
            <div className="font-semibold text-slate-700">Cliente</div>
            <div>{movement.customer_name || '—'}</div>
            {movement.customer_phone && <div className="text-slate-500">{movement.customer_phone}</div>}
          </div>
        )}
      </div>
